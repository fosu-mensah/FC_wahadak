...
function InternationalNews() {
    ...
    const loadNews = async (page) => {
        const data = await fetchNewsByCategory('international', page, pageSize);
    ...
    return (
        <Fragment>
            <Breadcrumb parent="News / International" title="국제 실축 뉴스" />
            <Container fluid={true} className="mt-3">
                {news.map(item => (
                    <Row key={item.news_id} className="mb-4">
                        <Card>
                            <CardHeader className="b-t-primary border-3">
                                <Link to={`${process.env.PUBLIC_URL}/news/international/${item.news_id}`}>
