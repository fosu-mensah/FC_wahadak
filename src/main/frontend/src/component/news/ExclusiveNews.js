...
function ExclusiveNews() {
    ...
    const loadNews = async (page) => {
        ...
        setNews(data.articles); // 데이터 구조에 맞게 수정
        setTotalPages(Math.ceil(data.totalCount / pageSize)); // 총 페이지 수 계산
    ...
    return (
        <Fragment>
            <Breadcrumb parent="News / Exclusive" title="FourFourTwo 독점 뉴스" />
